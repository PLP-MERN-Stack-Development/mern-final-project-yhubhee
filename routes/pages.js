...
router.get('/ui/search', validateTokens, (req, res) => {
    ...
    const sql = `SELECT doctor_id, doc_img, doc_name, specialty, about_doctor FROM doctors`;
    const sqlSchedule = `
    SELECT doctor_id, appointmentDate, appointmentTime, appointmentEndtime, fullname
    FROM appointment
  ...
  `;
    ...
    db.getConnection((err, connection) => {
        ...
        connection.query(sql, (err, doctors) => {
            ...
            connection.query(sqlSchedule, (err, schedules) => {
                // always release connection once all DB work is done
                connection.release();

                if (err) {
                    console.error('Database query error (schedules):', err);
                    return res.status(500).send('Failed to fetch doctor schedules');
                }

                // Render with explicit arrays (safe defaults)
...
// Appointment
router.get('/ui/appointment', validateTokens, (req, res) => {
    const { firstname, lastname, patient_id } = req.user;
    ...
    const sqlDoctors = `
    SELECT doc_img, doc_name, specialty, about_doctor, doctor_id
    FROM doctors
    WHERE status = 'top'
  `;
    const sqlSchedule = `
  ...
  `;
    ...
    db.getConnection((err, connection) => {
        ...
        connection.query(sqlDoctors, (err, doctors) => {
            if (err) {
                console.error('Database query error (doctors):', err);
                connection.release();
                return res.status(500).send('Failed to fetch doctors');
            }
...
router.get('/ui/book_appointment', validateTokens, (req, res) => {
    ...
    const doctor = req.query.doctor.trim() || 'Unknown Doctor';
    const specialty = req.query.specialty.trim();

    console.log('Specialty received:', specialty, typeof specialty);

    const sql = `SELECT doc_img, doc_name, specialty, about_doctor FROM doctors WHERE specialty = ? AND doc_name <> ?`;
    ...
    db.getConnection((err, connection) => {
        ...
        connection.query(sql, [specialty, doctor], (err, doctors) => {
            if (err) {
                console.error('Database query error:', err);
                connection.release();
                return res.status(500).send('Failed to fetch doctors');
            ...
            console.log('Related doctors (excluding current):', doctors);

            res.render('ui/book_appointment', {
                doctor,
                specialty,
                relatedDoctors: doctors || null,
            });
            connection.release();
        });
    });
...
router.get('/patients/booked-appointment', validateTokens, (req, res) => {
    ...
    const sql = `
       select a.doctor, d.specialty, a.fullname, a.appointmentDate, a.appointmentTime,a.appointmentEndtime, d.email,a.appointment_id, a.doctor_id, a.status from appointment a join doctors d on a.doctor_id = d.doctor_id WHERE patient_id = ? ORDER BY a.appointmentDate Desc, a.appointmentTime Desc;
    `;
    db.getConnection((err, connection) => {
        if (err) {
            console.error('Database connection error:', err);
...
router.get('/doctor/doc_appointment', doctorvalidateTokens, (req, res) => {
    ...
    const sql = `
 SELECT a.fullname, p.symptoms, a.appointmentDate, a.appointmentTime, a.doctor, p.email, a.appointment_id, a.patient_id, a.status FROM appointment a JOIN patients p ON a.patient_id = p.patient_id WHERE doctor_id = ? ORDER BY a.appointmentDate Desc, a.appointmentTime Desc; 
    `
...
router.get('/doc_schedule', doctorvalidateTokens, (req, res) => {
    const { doc_name, doctor_id } = req.user;
    const sql = `
 SELECT fullname, appointmentDate, appointmentTime FROM appointment WHERE doctor_id = ? AND STATUS = 'scheduled';
    `
    db.query(sql, [doctor_id], (err, results) => {
        if (err) {
            console.error('Database connection error:', err);
        ...
        // console.log(results)
        res.render('doctor/doc_schedule', {
            schedule: results || [],
            user: req.user,
        });
        console.log(results)
